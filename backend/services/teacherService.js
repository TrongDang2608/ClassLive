...
class TeacherService {
  ...
  async getProfile(teacherId) {
    const cacheKey = `classlive:teacher:profile:${teacherId}`;
  ...
  async getDashboardStats(teacherId) {
    const cacheKey = `classlive:teacher:stats:${teacherId}`;
  ...
  async getAssignedLessons(teacherId, page = 1, limit = 10, filters = {}) {
    ...
    const cacheKey = `classlive:teacher:lessons:${teacherId}:${pageNum}:${limitNum}:${filterKey}`;
  ...
  async getLessonDetail(lessonId, teacherId) {
    const cacheKey = `classlive:teacher:lesson_detail:${teacherId}:${lessonId}`;
    return await cacheService.remember(cacheKey, 1800, async () => {
